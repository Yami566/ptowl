 ...
 */
async function verifyFirebaseToken(idToken: string, projectId: string): Promise<FirebaseTokenPayload | null> {
  try {
    ...
    const jwks = await jwksResponse.json() as { keys: Array<{ kid: string; n: string; e: string; kty: string; alg: string }> };
    ...
    const payload = JSON.parse(atob(parts[1]!.replace(/-/g, '+').replace(/_/g, '/'))) as FirebaseTokenPayload;
...
authRoutes.post('/firebase', async (c) => {
  try {
    ...
    if (!rawPhone) {
      return c.json({ ok: false, error: { code: 'INVALID_TOKEN', message: 'No phone number in token' } }, 400);
    }
    ...
    if (!isNewUser) {
      await c.env.DB.prepare('INSERT INTO audit_log (id, user_id, action, detail, ip_address) VALUES (?, ?, ?, ?, ?)')
        .bind(crypto.randomUUID().replace(/-/g, ''), user.id, 'login_phone', phone, c.req.header('cf-connecting-ip') || '')
        .run();
    }
...
authRoutes.get('/approve/:token', async (c) => {
  try {
    ...
    if (Date.now() > expires) {
      return c.html(approvalPage('Link Expired', 'This approval link has expired. Please use the admin panel.', false));
    }

    // Verify HMAC signature
    const encoder = new TextEncoder();
    const hmacKey = await crypto.subtle.importKey('raw', encoder.encode(c.env.JWT_SECRET), { name: 'HMAC', hash: 'SHA-256' }, false, ['verify']);
    ...
    if (!result.meta.changes) {
      ...
      const existing = await c.env.DB.prepare('SELECT status, user_alias FROM users WHERE id = ?')
        .bind(userId).first<{ status: string; user_alias: string }>();
      if (existing?.status === 'approved') {
        return c.html(approvalPage('Already Approved', `${existing.user_alias || 'This user'} was already approved.`, true));
      }
      return c.html(approvalPage('User Not Found', 'Could not find a pending user with this ID.', false));
    }

    // Get the alias for the confirmation
    const user = await c.env.DB.prepare('SELECT user_alias FROM users WHERE id = ?')
      .bind(userId).first<{ user_alias: string }>();

    // Audit log
    await c.env.DB.prepare('INSERT INTO audit_log (id, user_id, action, detail, ip_address) VALUES (?, ?, ?, ?, ?)')
      .bind(crypto.randomUUID().replace(/-/g, ''), userId!, 'approve_user_link', 'One-click approval', c.req.header('cf-connecting-ip') || '')
      .run();

    return c.html(approvalPage('Approved!', `${user?.user_alias || 'User'} has been approved and can now access PtOwl.`, true));
  } catch (err) {
    console.error('Approval error:', err instanceof Error ? err.message : 'Unknown');
    return c.html(approvalPage('Error', 'Something went wrong. Try the admin panel instead.', false));
  }
});
