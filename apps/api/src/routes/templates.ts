...
templateRoutes.get('/', async (c) => {
  try {
    ...
    return c.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch templates' } },
      500,
    );
  }
});
...
templateRoutes.put('/:id', async (c) => {
  try {
    ...
    const existing = await c.env.DB.prepare(
      'SELECT id, user_id, hotkey, name, sessions_per_week, duration_weeks, default_time, is_active, sort_order, created_at, updated_at FROM templates WHERE id = ? AND user_id = ?',
    )
    ...
    if (body.name !== undefined) {
      if (typeof body.name !== 'string' || body.name.length > 100) {
        return c.json(
          { ok: false, error: { code: 'INVALID_INPUT', message: 'Name too long (max 100 chars)' } },
          400,
        );
      }
      ...
      if (sanitizedName.length === 0) {
        return c.json(
          { ok: false, error: { code: 'INVALID_INPUT', message: 'Name cannot be empty' } },
          400,
        );
      }
      updates.push('name = ?');
    ...
    if (body.sessions_per_week !== undefined) {
      if (
        !Number.isInteger(body.sessions_per_week) ||
        body.sessions_per_week < 1 ||
        body.sessions_per_week > 7
      ) {
        return c.json(
          { ok: false, error: { code: 'INVALID_INPUT', message: 'Sessions per week must be 1-7' } },
          400,
        );
      }
    ...
    if (body.duration_weeks !== undefined) {
      if (
        !Number.isInteger(body.duration_weeks) ||
        body.duration_weeks < 1 ||
        body.duration_weeks > 52
      ) {
        return c.json(
          { ok: false, error: { code: 'INVALID_INPUT', message: 'Duration must be 1-52 weeks' } },
          400,
        );
      }
    ...
    if (body.is_active !== undefined) {
      if (body.is_active !== 0 && body.is_active !== 1) {
        return c.json(
          { ok: false, error: { code: 'INVALID_INPUT', message: 'is_active must be 0 or 1' } },
          400,
        );
      }
    ...
    return c.json(
      { ok: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to update template' } },
      500,
    );
  }
});
...
templateRoutes.delete('/:id', async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare('DELETE FROM templates WHERE id = ? AND user_id = ?')
      .bind(templateId, user.id)
      .run();
