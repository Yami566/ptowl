...
export function OwlLogo({ size = 'md', linkTo }: OwlLogoProps) {
  ...
  const fontSize = size === 'sm' ? 'clamp(0.9rem, 2.5vw, 1.1rem)' : size === 'lg' ? 'clamp(2rem, 5vw, 3.2rem)' : 'clamp(1.1rem, 3vw, 1.5rem)';
  const oSize = size === 'sm' ? 'clamp(1rem, 3vw, 1.3rem)' : size === 'lg' ? 'clamp(2.4rem, 6vw, 3.8rem)' : 'clamp(1.3rem, 3.5vw, 1.8rem)';
