...
│   │   ├── src/
│   │   │   ├── crypto/         # JWT, CSRF, password hashing
...
```
1. Login response sets csrf_token cookie (readable by JavaScript)
2. Client reads csrf_token from document.cookie
3. Client sends X-CSRF-Token header on every state-mutating request
4. Server middleware validates: header value matches cookie value
5. Both are HMAC-SHA256 signed with JWT_SECRET
```
...
- Routes: ptowl.com/api/*, www.ptowl.com/api/*
- D1 binding: ptowl-db
...
**Rationale:** httpOnly cookies cannot be read by JavaScript, preventing XSS-based token theft. Combined with SameSite=Lax, this also provides CSRF protection for GET requests. Signed CSRF tokens handle POST/PUT/DELETE.
...
*This document is maintained alongside the codebase and updated as architecture evolves.*
