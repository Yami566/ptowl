...
and clinic logos in object storage (R2). Sign-in is handled by Firebase
(Google + email-link). Today the app is in beta with a small handful of
clinics. Brand: green and orange, owl mascot, sports-legend aliases as
a privacy failsafe instead of patient names.
...
### 3. Backups and log retention

**What:** Once a day, an automated job exports the database to object
storage as a snapshot. Old audit-log rows beyond a retention window
get archived (kept in storage, removed from the live DB).

**Why:** If the live database ever corrupts or someone accidentally
deletes data, we have a yesterday-good copy to restore from. Audit log
rotation keeps queries fast and storage costs predictable.

**How it works in plain terms:** Cloudflare lets us schedule a small
job to run on a clock (their built-in scheduling tool). The job dumps
the current database to a file, writes that file to our object
storage, and deletes any audit-log rows older than ~90 days. No human
involvement after setup.

**State today:** Not built. This is the easiest of the three to ship
because it's just a scheduled job that touches infrastructure we
already have.
...
```
                    Browser
                       ...
                       ▼
              ┌──────────────────┐
              │  Frontend Worker │  ──────── Firebase Auth (Google + email-link)
              │  (ptowl.com)     │
              └──────────────────┘
                       │
                       ▼
...
Everything lives inside Cloudflare except the Firebase Auth
dependency. We don't have Queues, Durable Objects, or Workflows wired
up yet, and at our scale we shouldn't add them — plain Workers + D1

- R2 + scheduled jobs cover all three priorities above.
